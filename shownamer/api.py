# api.py - api stuff mostly (tvmaze, omdb, tmdb, kitsu)
...
_tmdb_key_cache: dict[str, bool | str | None] = {
    "checked": False,
    "key": None,
}
_omdb_key_cache: dict[str, bool | str | None] = {
    "checked": False,
    "key": None,
}

_anime_search_cache: dict[str, list[dict[str, Any]]] = {}
_anime_episode_cache: dict[tuple[str, int], dict[str, Any] | None] = {}
...
def get_tmdb_key() -> str | None:
    ...
    print(
        "Get a free TMDb API key (v3 auth) from https://www.themoviedb.org/settings/api"
    )
...
def _tmdb_certification(
    release_dates: dict[str, Any] | None,
    country: str = "US",
) -> str | None:
    ...
    for entry in results:
        if entry.get("iso_3166_1") == country:
            for release_date in entry.get("release_dates", []):
...
def fetch_tmdb_metadata(
    ...
    actors = ", ".join(person["name"] for person in cast[:3]) or "N/A"
    ...
    if imdb_id and omdb_api_key:
        enrichment = fetch_omdb_metadata_by_imdb_id(
            imdb_id,
            omdb_api_key,
        )

        if enrichment:
            metadata["Ratings"] = enrichment.get(
                "Ratings",
                [],
            )

            metadata["Awards"] = enrichment.get(
                "Awards",
                "N/A",
            )

            if metadata["BoxOffice"] == "N/A":
                metadata["BoxOffice"] = enrichment.get(
                    "BoxOffice",
                    "N/A",
                )

            if metadata["Rated"] == "N/A":
                metadata["Rated"] = enrichment.get(
                    "Rated",
                    "N/A",
                )
...
def search_anime(query: str) -> list[dict[str, Any]]:
    if query in _anime_search_cache:
        return _anime_search_cache[query]
    ...
    _anime_search_cache[query] = results
...
def get_anime_episode(
    ...
    cache_key = (anime_id, episode_number)

    if cache_key in _anime_episode_cache:
        return _anime_episode_cache[cache_key]

    try:
        response = requests.get(
            f"{KITSU_URL}/anime/{anime_id}/episodes",
            params={
                "filter[number]": episode_number,
                "page[limit]": 1,
            },
            timeout=(5, 10),
        )
        ...
        if not data:
            _anime_episode_cache[cache_key] = None
            return None
        ...
        title = (
            ...
            or attributes.get("titles", {}).get("en_jp")
            or f"Episode {number}"
        )
        ...
        _anime_episode_cache[cache_key] = episode
